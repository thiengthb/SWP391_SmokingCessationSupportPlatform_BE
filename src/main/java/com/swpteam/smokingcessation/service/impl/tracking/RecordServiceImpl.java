...
public class RecordServiceImpl implements IRecordService {
    ...
    public RecordResponse createRecord(RecordCreateRequest request) {
        ...
        boolean existed = recordRepository.existsByAccountIdAndDateAndIsDeletedFalse(request.getAccountId(), request.getDate());
        if (existed) {
            Record record = recordRepository.findByAccountIdAndDateAndIsDeletedFalse(request.getAccountId(), request.getDate())
                    .orElseThrow(() -> new AppException(ErrorCode.HEALTH_RECORD_NOT_FOUND));
